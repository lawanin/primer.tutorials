...
Continue your pipe and add `ggplot()`. Map `price` to the x-axis, and `carat` to the y-axis. Then, add the layer `geom_point()`.
...
Within `aes()` in `ggplot()`, add the argument `text` and set this equal to the function `paste`. This will return an error because we haven't assigned anything to `text`. We will do this in the following exercises. 
